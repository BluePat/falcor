...
// Helper function to check if an object is safe to mutate (not a prototype pollution vector)
function isSafeToMutate(obj) {
    if (obj == null || typeof obj !== 'object') {
        return true;
    }
    // Prevent mutations on dangerous prototype objects and constructor
    var proto = Object.prototype;
    // Check if obj is __proto__, constructor, or any built-in prototype
    if (obj === proto || 
    ...
    // Additional check: prevent mutation if obj.constructor is a built-in
    try {
        var str = obj.toString();
        if (str === '[object Object]' && obj.constructor === Object && obj === obj.constructor.prototype) {
            return false;
        }
    } catch (e) {
        // If toString throws, be safe and allow mutation
    }
    return true;
}
                    ...
                    if (Array.isArray(key)) {
                        ...
                        if (key != null && typeof key === 'object') {
                            if (isSafeToMutate(key)) {
                                ...
                                key = key.offset !== void 0 ? key.offset : (key.from !== void 0 ? key.from : 0);
                            }
                        }
                    } else {
                        if (isSafeToMutate(key)) {
                ...
                for (; column >= 0; --column) {
                    ...
                    if ( // TODO: replace this with a faster Array check.
                        ...
                        if (isSafeToMutate(key)) {
                            ...
                            // Cannot safely mutate, skip this path
                            continue ascending;
                        }
                    }
                    if (isSafeToMutate(key)) {
