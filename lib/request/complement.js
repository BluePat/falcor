...
/**
 * Safely gets a property from an object, preventing prototype pollution.
 * Returns undefined if the key accesses the prototype chain.
 * @private
 */
function safeGetProperty(obj, key) {
    ...
    // Only return the property if it's an own property, not from the prototype chain
...
function findPartialIntersections(requestedPath, optimizedPath, requestTree) {
    ...
    for (i = 0; requestTree && i < -depthDiff; i++) {
        requestTree = safeGetProperty(requestTree, optimizedPath[i]);
    }
...
function recurse(requestedPath, optimizedPath, currentTree, depth, rCurrentPath, oCurrentPath) {
    ...
    for (; depth < oPathLen; ++depth) {
        ...
        if (key && keyType === "object") {
            ...
            while (!note.done) {
                var nextTree = safeGetProperty(currentTree, innerKey);
            ...
            currentTree = safeGetProperty(currentTree, key);
