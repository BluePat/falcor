...
/**
 * Safely access object property, preventing prototype pollution by blocking
 * access to __proto__, constructor, and prototype properties.
 * @param {Object} obj - The object to access
 * @param {*} key - The property key
 * @returns {*} The property value or undefined if key is dangerous
 */
function safeGet(obj, key) {
    // Block access to dangerous property names that can lead to prototype pollution
...
function findPartialIntersections(requestedPath, optimizedPath, requestTree) {
    ...
    for (i = 0; requestTree && i < -depthDiff; i++) {
        requestTree = safeGet(requestTree, optimizedPath[i]);
    }
...
function recurse(requestedPath, optimizedPath, currentTree, depth, rCurrentPath, oCurrentPath) {
    ...
    for (; depth < oPathLen; ++depth) {
        ...
        if (key && keyType === "object") {
            ...
            while (!note.done) {
                var nextTree = safeGet(currentTree, innerKey);
            ...
            currentTree = safeGet(currentTree, key);
