...
var isSafeKey = require("./util/isSafeKey");
...
module.exports = function onValue(model, node, seed, depth, outerResults,
    ...
    if (isJSONG) {
        ...
        for (i = 0, len = optimizedLength - 1; i < len; i++) {
            ...
            if (!isSafeKey(key)) {
                // Cannot safely continue path traversal, abort this path
                return;
            }
        ...
        // Only assign if key is safe to prevent prototype pollution
        if (isSafeKey(key)) {
    ...
    // reporting the value.
        ...
        if (valueNode !== undefined) {
            ...
            // Only assign if prevK is safe to prevent prototype pollution
            else if (isSafeKey(prevK)) {
