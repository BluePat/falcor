...
var isPrototypePollutionKey = require("./util/isPrototypePollutionKey");
...
module.exports = function onValue(model, node, seed, depth, outerResults,
    ...
    if (isJSONG) {
        ...
        // Check for prototype pollution keys in the path
        for (i = 0, len = optimizedLength; i < len; i++) {
            if (isPrototypePollutionKey(optimizedPath[i])) {
                // Abort processing this path if it contains prototype pollution keys
                return;
            }
        }
    ...
    // reporting the value.
        ...
        // Check for prototype pollution keys in the path
        for (i = 0; i < depth; i++) {
            if (isPrototypePollutionKey(requestedPath[i])) {
                // Abort processing this path if it contains prototype pollution keys
                return;
            }
        }
