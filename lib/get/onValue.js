...
module.exports = function onValue(model, node, seed, depth, outerResults,
    ...
    if (isJSONG) {
        ...
        // Check all keys in the path for safety to prevent prototype pollution
        var pathIsSafe = true;
        for (i = 0, len = optimizedLength; i < len; i++) {
            if (!isSafeKey(optimizedPath[i])) {
                pathIsSafe = false;
                break;
            }
        }
        
        // Only build the path if all keys are safe
        if (pathIsSafe) {
